(function($){
    (async function(){
        ...
        var autocomplete = new google.maps.places.PlaceAutocompleteElement({inputElement: input.get(0), types:["(regions)"]});
        ...
        function clearentry(e){
            if(isMultiple)
                return;
            input.data("clearedvalue",input.val()).val('');
        }

        autocomplete.addListener('gmp-placeselect', isMultiple ? onPlaceAdd : onPlaceChange);
