...
<div class="row form-row geo-selectregion">
	...
	<div class="col-sm-#args.widthsplit[2]#">
		<div id="countries" class="mb-1">
			#renderview(view:"_form_bs/select", args:{widthsplit:"", name:"countryID", options:getInstance("RegionService@geonames").getCountryOptions(lang:getFwLang(), select:args.subsetcountries, withpreferred:args.withpreferred), default:selectedcountry, formrow:false})#
		</div>
		<cfloop array="#args.countriesRequireAdmin#" item="country">
			<div class="#selectedcountry==country?'':'inithidden'# row-space-1 geo-subselect geo-#country#">
				#renderview(view:"_form_bs/select", args:{widthsplit:"", name:"admin1_#country#", options:getInstance("RegionService@geonames").getAdminOptions(getFwLang(), country), default:event.getValue("admin1_#country#",args.geo.getID()?:""), formrow:false})#
			</div>
		</cfloop>
		<cfif args.subsetcountries>
			<div class="#selectedcountry=='other'?'':'inithidden'# row-space-1 geo-subselect geo-other">
				#renderview(view:"_form_bs/select", args:{widthsplit:"", name:"continentID", options:getInstance("RegionService@geonames").getContinentOptions(getFwLang()), default:event.getValue("continentID",args.geo.getID()?:""), formrow:false})#
			</div>
		</cfif>
	</div>
